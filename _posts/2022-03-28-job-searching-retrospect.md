...
지원한 회사들은 모두 시리즈 B부터 그 이상, IPO를 앞두고 있는 규모있는 회사까지 지원했습니다. 모두 자체 서비스 스타트업들입니다.
...
스스로에게 가장 익숙한 스택들을 가지고 빠르게 요구사항을 충족해나가면서, 조금이라도 더 코드 퀄리티를 생각하면서 구현해야한다는 생각입니다. 정해진 시간 안에 최대한 많은 점수를 얻는게 목표니까요.

그리고 욕심내지 말고, 과제가 지시하는 요구사항을 꼼꼼히 확인해 최소 사항들을 충족하는데 집중해야 합니다. 프로젝트를 더 멋지게 만든다거나, 성능, 사용성 개선은 시간이 남을 때 진행해야 합니다.
...
문제를 풀면서 말을 자유로이 하기가 힘들다면, **코드를 작성하며 생각한 바를 주석으로 남기는 방법**도 좋은 것 같습니다. 인터뷰이분도 주석을 확인하면서 지원자의 생각의 흐름을 확인할 수 있고, 적당한 코멘트를 남겨주실 수 있습니다.
...
이력서 기반 면접같은 경우는 이력서에 적은 경험을 바탕으로, 인터뷰이분들의 입장에서 무엇을 궁금해하실까 생각하면서 파생 질문들을 생각이 더 나지 않을때까지 정리해나갔습니다. 이 작업을 하면서 특정 질문에 확실하게 대답해야할 포인트들을 추출할 수 있었던 것 같습니다.

이번 구직에서 기술 면접에 들어갈 때에는 `~는 뭔가요? ~에 대해 설명해주세요` 식으로 받은 질문들보다는 이력서 기반 면접 질문들을 더 많이 받았습니다. 이력서에 쓰인 경험들을 바탕으로 `왜 이렇게 했는지`, `어떤 의사결정 과정을 거쳤는지` 같은 질문이 더 많았아요. 저번 구직과는 다르게 미약하게나마 경력이 생겨서 그런가보다 했습니다.
...
구직을 시작하기 전에 저는 **기본급 + 성과급에 대한 하한가**를 설정했습니다. 무조건 이정도는 받아야 한다는 기준이었는데요.

그런데 역시... ~~광란의 개발자 채용 시대~~라서 그런지 제가 원하는 처우에는 굉장히 빨리 도달할 수 있었습니다. 여러 회사의 오퍼를 받으면서 저에 대한 회사들의 지불 용의, 저의 시장가를 측정하면서 희망 처우의 기준을 잡아갔던 것 같습니다.
