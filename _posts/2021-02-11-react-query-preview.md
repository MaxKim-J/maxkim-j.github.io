...
- 한줄요약 : **비동기 요청의 데이터 무결함에 대한 책임을 개발자가 아니라 React 앱 자체가 책임지게 하는 라이브러리**
- **비동기 요청의 무결함** : 비동기 요청 데이터가 view에서 필요할때, 그 전에 비동기 요청이 동작하여 데이터를 참조할 수 있거나, 에러로 인한 예외처리가 보장되는 상황
- **과정이 아닌 결과의 무결함** : 대부분의 경우 요청, 요청 완결 직후 데이터 참조 혹은 예외처리가 이루어졌던 비동기 요청의 관행?에서 벗어나 라이브러리가 알아서 캐싱과 리패칭을 해내면서 **요청 시점이 데이터 참조 시점 직전이 아니더라도 view에서 데이터가 필요할때 해당 데이터를 참조할 수 있음을 보장함**
- **Context API** : context를 사용해 비동기, server state를 관리하는 전역 계층을 제공해 비동기 요청을 관리하는 라이브러리.
- **덜 선언적** : saga에서처럼 비동기 관련한 성공, 실패 액션 하나하나를 모두 선언하여 장황하게 정리할 필요가 없고, 프로미스를 리턴하는 비동기 요청 함수와 엔드포인트만 정리해놓으면 `useQuery`를 통한 비동기 요청 로직을 컴포넌트 곳곳에다가 흩뿌려놓아도 Context가 고유한 key를 이용해 해쉬하여 정리해서 비동기 로직들을 관리할 수 있음.
- 속을 너무 뜯어보고 싶음... 어떻게 캐싱이랑 리패칭, 시간 간격을 두고 인스턴스를 해제한다거나 이런걸 다 구현했는지....🧐
...
- Server-State : 서버에서 가져오는 데이터들도 하나의 상태로 보아야 한다.
...
- React 자체가 데이터를 패칭해오거나 업데이트 하는 옵션을 제공하지 않기 때문에 원래 React 개발자들은 각자의 방식으로 http 통신 로직을 짜야 했다. ⇒ React Query는 훅 2개를 제공해 컴포넌트에서 http 통신 을 쉽게 하고, 과정과 결과를 쉽게 확인할 수 있는 수단을 제공
- Redux같은 전역 상태관리 라이브러리들이 클라이언트 상태값에 대해서는 잘 작동하지만, 서버 상태에 대해서는 그렇게 잘 작동하지 않는다. Server State는 Client State와 완전 다르기 때문이다. ⇒ 앱을 감싸는 Context 계층에서 캐싱과 리패치, 데이터가 oudated 되었을때의 리패치 등을 수행해 http 통신을 최적화
...
- Query들은 4개의 상태를 가지며, useQuery가 반환하는 객체의 프로퍼티로 어떤 상태인지 확인이 가능하다.
    ...
    3. stale : 썩은? 쿼리. 인스턴스가 존재는 하는데 이미 패칭이 완료된 쿼리. 특정 쿼리가 stale된 상태에서 같은 쿼리를 useQuery로 또 호출해 마운트를 시도한다면 캐싱된 데이터가 반환된다.
    4. inactive : active 인스턴스가 하나도 없는 쿼리. 5분 간격으로 아예 삭제된다.
- 어떻게 inactive가 되는가? : 간단한 실제 예로는, 앱에서 내비게이션으로 인해 다른 컴포넌트가 랜더링이 되었는데, 해당 컴포넌트에 전 컴포넌트에서 마운트되었던 쿼리를 똑같이 마운트하는 useQuery나 useMutation 선언문이 없을때 이전 컴포넌트의 stale 쿼리들은 inactive 상태가 된다.
- 예상할 수 있듯 쿼리의 존재는 컴파일 과정에서 한번에 싹 다 파악되지 않는다. 앱 실행 중 컴포넌트 안에 useQuery나 useMutation을 만나 호출하는 시점에 그 때 쿼리 인스턴스가 마운트되고 파악된다. 그래서 실행 중에 백그라운드에서 관리하는 쿼리 인스턴스의 개수가 계속 달라짐.
- stale 상태가 된 쿼리는 다음과 같은 상황에서 백그라운드에서 다시 패치된다.
